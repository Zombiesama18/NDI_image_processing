...
@register_model
def vit_tiny(pretrained=False, in_chans=3, num_classes=21843, **kwargs):
    if not pretrained:
        model = VisionTransformer(
            patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), in_chans=in_chans, num_classes=num_classes, **kwargs)
        return model
    model = timm.create_model('vit_tiny_patch16_224_in21k', pretrained=True)
    if in_chans != 3:
        model.patch_embed = PatchEmbed(224, 16, in_chans, 192)
    if num_classes != 21843:
        if num_classes == 0:
            model.head = nn.Identity()
        else:
            model.head = nn.Linear(192, num_classes, bias=True)
    return model


@register_model
def vit_small(pretrained=False, in_chans=3, num_classes=21843, **kwargs):
    if not pretrained:
        model = VisionTransformer(
    ...
    if in_chans != 3:
        model.patch_embed = PatchEmbed(224, 16, in_chans, 384)
    if num_classes != 21843:
        if num_classes == 0:
            model.head = nn.Identity()
        else:
            model.head = nn.Linear(384, num_classes, bias=True)
    return model


@register_model
def vit_base(pretrained=False, in_chans=3, num_classes=21843, **kwargs):
    if not pretrained:
        model = VisionTransformer(
    ...
    if in_chans != 3:
        model.patch_embed = PatchEmbed(224, 16, in_chans, 768)
    if num_classes != 21843:
        if num_classes == 0:
            model.head = nn.Identity()
        else:
...
@register_model
def vit_large(pretrained=False, in_chans=3, num_classes=21843, **kwargs):
    if not pretrained:
        model = VisionTransformer(
    ...
    if in_chans != 3:
        model.patch_embed = PatchEmbed(224, 16, in_chans, 1024)
    if num_classes != 21843:
        if num_classes == 0:
            model.head = nn.Identity()
        else:
