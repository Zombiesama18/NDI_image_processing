...
def draw_graph(metrics, num_epochs: int, metrics_name: (list, tuple)):
    ...
    fig, axes = plt.subplots((len(metrics) + 1) // 2,
                             2, figsize=(15, 4 * (len(metrics) + 1) // 2))
...
def train_sample():
    ...
    train_metrics = HistoryRecorder(
        ['Train Loss', 'Train Acc', 'Val Loss', 'Val Acc'], list(parameters.keys()))
    for images in k_fold_train_validation_split(ORIGINAL_IMAGE, TARGET_IMAGE, k):
        train_dataset = ThreeChannelNDIDatasetContrastiveLearningWithAug(
            images, False)
        val_dataset = ThreeChannelNDIDatasetContrastiveLearningWithAug(
            images, True)
