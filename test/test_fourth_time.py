...
def init_wandb():
    ...
    wandb.init(project="Sperated-Categories-Angles", name='Test-fourth-time-transform-random-rotation', config={'random_seed': current_seed})

def NDI_transforms(img, labels):
    class_name, angle_label = labels
    angle_label = 90 * angle_label
    rotate_angle_range = (-90 - angle_label, 90 - 0.0001 - angle_label)
    rotate_func = RandomRotationWithAngle(rotate_angle_range)
    rotated_img = rotate_func(img)
    to_tensor_func = torchvision.transforms.ToTensor()
    rotated_img = to_tensor_func(rotated_img)
    rotated_angle = rotate_func.get_rotate_angle()
    angle_label = rotated_angle + angle_label
    angle_label = angle_label / 90
    return rotated_img, (class_name, angle_label)
    

train_set, val_set = create_train_val_dataset('../datasets/NDI_images/annotation.csv', train_transforms=NDI_transforms)
